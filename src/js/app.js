var bootleaf = {
  ...
  basemaps: [
    ...
    },
    {
      ...
      type: "mapbox",
      theme: "streets-satellite",
      label: "Streets with Satellite (MapBox)",
    },
    {
      id: "MapboxHighContrast",
      type: "mapbox",
      theme: "high-contrast",
      label: "High-contrast (MapBox)",
    },
    {
      id: "esriStreets",
      type: "esri",
    ...
    },
    {
      ...
      type: "esri",
      theme: "Gray",
      label: "Light gray (ArcGIS)",
    },
    {
      id: "esriTopographic",
      type: "esri",
      theme: "Topograhic",
      label: "Topographics (ArcGIS)",
    },
    {
      id: "esriImagery",
      type: "esri",
    ...
    },
    {
      ...
      type: "esri",
      theme: "ShadedRelief",
      label: "Shaded relief (ArcGIS)",
    },
    {
      id: "esriTerrain",
      type: "esri",
      theme: "Terrain",
      label: "Terrain (ArcGIS",
    },
    {
      id: "esriDarkGray",
      type: "esri",
      theme: "DarkGray",
      label: "Dark gray (ArcGIS)",
    },
    {
      id: "esriNationalGeographic",
      type: "esri",
      theme: "NationalGeographic",
      label: "National Geographic (ArcGIS)",
    },
    {
      id: "esriOceans",
      type: "esri",
...
// La función `beforeMapLoads` permite insertar propiedades personalizadas para manipular el archivo de configuración u
// otras operaciones necesarias, sin modificar el código fuente de Bootleaf.
// Si el archivo de configuración especifica `requireArcGISLogin: true`, se mostrará la ventana modal de inicio de sesión.
// Una vez que el usuario haya iniciado sesión correctamente, se llamará a la función `beforeMapLoads`.
$(document).ready(function () {
  try {
    ...
    $.growl.error({
      message:
        "There was a problem running the BeforeMapLoads custom code: " +
        error.message,
      fixed: true,
    });
  }
...
function loadMap() {
  ...
  if (typeof config === "undefined") {
    $.growl.error({
      message: "Error - the configuration file is not found!",
      fixed: true,
    });
    return null;
  }
    ...
    try {
      ...
      if (layerConfig.tokenRequired) {
        if (config.token !== undefined) {
          ...
          $.growl.error({
            message: "A token is required for this layer but was not provided",
          });
        }
      }
      ...
      if (layerType === "agsFeatureLayer") {
        ...
        if (
          layerConfig.outFields !== undefined &&
          layerConfig.outFields.length > 0
        ) {
        ...
        if (layerConfig.popup) {
          layer.on("click", function (evt) {
            ...
            for (key in evt.layer.feature.properties) {
              // Ignore ID fields
              if (
                ["FID", "OBJECTID", "OID", "id", "ID", "ObjectID"].indexOf(
                  key
                ) < 0
              ) {
        ...
        if (layerConfig.tooltipField !== undefined) {
          layer.on("mouseover", function (evt) {
            try {
              var tooltipContent =
                evt.layer.feature.properties[this.layerConfig.tooltipField];
        ...
        if (
          layerConfig.popup !== undefined ||
          layerConfig.tooltipField !== undefined
        ) {
        ...
        $.ajax({
          ...
          error: function (jqXHR, textStatus, error) {
            $.growl.warning({
              message:
                "There was a problem fetching the features for " +
            ...
            if (jqXHR.layer !== undefined) {
              try {
                ...
                bootleaf.visibleLayers.splice(
                  bootleaf.visibleLayers.indexOf(jqXHR.layer.id),
                  1
                );
                bootleaf.TOCcontrol.removeLayer(jqXHR.layer);
              } catch (err) {
                console.log(
                  "There was a problem removing this layer from the map"
                );
              }
            }
          },
      ...
      if (
        layerConfig.queryWidget !== undefined &&
        layerConfig.queryWidget.queries &&
        layerConfig.queryWidget.queries.length > 0
      ) {
        var queries = [];
        for (
          var queryIdx = 0;
          queryIdx < layerConfig.queryWidget.queries.length;
          queryIdx++
        ) {
  ...
  if (config.start.maxZoom === undefined) {
    config.start.maxZoom = 19;
  } // Required for the Marker Clusterer
  ...
  if (config.controls) {
    if (config.controls.zoom) {
      if (navigator.userAgent.match(/iPad/i) === null) {
        bootleaf.zoomControl = L.control
          .zoom({
    ...
    if (config.controls.TOC) {
      bootleaf.tocOptions["position"] =
        config.controls.TOC.position || "topright";
      ...
      bootleaf.TOCcontrol = L.control.groupedLayers(
        null,
        bootleaf.layerTOC,
        bootleaf.tocOptions
      );
    ...
    if (config.controls.history) {
      try {
        bootleaf.historyControl = new L.HistoryControl(
          config.controls.history
        ).addTo(bootleaf.map);
      } catch (err) {
        $.growl.warning({
          message: "There was a problem enabling history on this application",
        });
      }
    }
    ...
    if (config.controls.leafletGeocoder !== undefined) {
      var geocoder = L.Control.Geocoder.nominatim(
        config.controls.leafletGeocoder
      );
      ...
      bootleaf.leafletGeocoder = L.Control.geocoder({
        ...
        placeholder:
          config.controls.leafletGeocoder.placeholder ||
    ...
    if (config.controls.bookmarks) {
      ...
      bootleaf.bookmarkControl = new L.Control.Bookmarks(bookmarkOptions).addTo(
        bootleaf.map
      );

      // Check if each bookmark has already been added via local storage, to avoid duplicate bookmarks
      localBookmarks = $.map(
        bootleaf.bookmarkControl._data || [],
        function (val, i) {
          return val.id;
        }
      );
      ...
      if (
        config.controls.bookmarks.places !== undefined &&
        config.controls.bookmarks.places.length !== undefined
      ) {
        for (
          var bmIdx = 0;
          bmIdx < config.controls.bookmarks.places.length;
          bmIdx++
        ) {
          var bm = config.controls.bookmarks.places[bmIdx];
          if ($.inArray(bm.id, localBookmarks) === -1) {
            bootleaf.map.fire("bookmark:add", {
              data: config.controls.bookmarks.places[bmIdx],
            });
          }
        }
      }
  ...
  $.map(bootleaf.basemaps || [], function (basemap, i) {
    if (
      ...
      $.inArray(basemap.id, config.basemaps) > -1
    ) {
      var html = "<li>";
      html +=
        '<a href="#" data-toggle="collapse" data-target=".navbase-collapse.in" class="dropdown-item liBasemap" data-type="' +
        basemap.type +
        '" data-theme="' +
        basemap.theme +
        '"' +
  ...
  if (
    config.basemaps === undefined ||
    (config.basemaps !== undefined && config.basemaps.length > 1)
  ) {
    $('*[data-basemapId="' + bootleaf.defaultBasemapId + '"]').addClass(
      "active"
    );
...
function reorderLayers() {
  ...
  for (var i = 0; i < bootleaf.layers.length; i++) {
    ...
    if (
      bootleaf.map.hasLayer(layer) &&
      layer.layerConfig &&
      layer.layerConfig.showOnTop
    ) {
      layer.bringToFront();
    }
  }
...
function addLayer(layer) {
  ...
  if (layerAdded === false) {
    ...
    if (
      config.controls &&
      config.controls.TOC &&
      config.controls.TOC.uncategorisedLabel
    ) {
  ...
  if (config.start.layers == undefined) {
    config.start["layers"] = [];
  }
...
function setBasemap(basemap) {
  if (bootleaf.basemapLayer) {
    bootleaf.map.removeLayer(bootleaf.basemapLayer);
  }
  var options = {
    maxZoomLevel: 23,
    maxZoom: 23,
  ...
  if (basemap.type === "esri") {
    ...
    options["attribution"] =
      "<a target='_blank' href='https://www.mapbox.com/about/maps/'>© Mapbox</a>, <a href='https://www.openstreetmap.org/copyright' target='_blank'>© OpenStreetMap</a>, <a href='https://www.mapbox.com/map-feedback/' target='_blank'>Improve this map</a>";
    var mapboxKey = config.mapboxKey || "";
    if (mapboxKey === "" || mapboxKey === undefined) {
      $.growl.warning({
        title: "Map Box error",
        message:
          "Ensure that you have specified a valid MapBox key in the config file",
      });
    }
    var mapboxTheme = basemap.theme || "streets";
    bootleaf.basemapLayer = L.tileLayer(
      "https://a.tiles.mapbox.com/v4/mapbox." +
        mapboxTheme +
    ...
    var options = {
      ...
      attribution:
        "<a href='https://www.microsoft.com/en-us/maps/ target='_blank'>Bing</a>",
    };
...
function updateIdentifyLayers() {
  ...
  bootleaf.identifyLayers = $.map(
    bootleaf.map._layers || [],
    function (val, i) {
...
function configureQueryWidget() {
  ...
  if (bootleaf.queryTasks) {
    if (bootleaf.queryTasks.length > 0) {
      ...
      $("#sidebarContents").html(
        "<p><span class='info'>There are no query-able layers in the map</span></p>"
      );
  ...
  if (
    bootleaf.drawControl._map === null ||
    bootleaf.drawControl._map === undefined
  ) {
...
function runQueryWidget() {
  ...
  var fieldType = $("#queryWidgetField option:selected")[0].dataset[
    "fieldtype"
  ];
  ...
  if (layer.type === "agsDynamicLayer" || layer.type === "agsFeatureLayer") {
    ...
    if ($("#chkQueryWithinMapExtent").is(":checked")) {
      ...
      if (bootleaf.queryPolygon.toGeoJSON().features.length > 0) {
        ...
        var geometry = {
          rings:
            bootleaf.queryPolygon.toGeoJSON().features[0].geometry.coordinates,
          spatialReference: { wkid: 4326 },
        };
        queryData.geometry = JSON.stringify(geometry);
    ...
    if ($("#chkQueryWithinMapExtent").is(":checked")) {
      ...
      var vertices =
        bootleaf.queryPolygon.toGeoJSON().features[0].geometry.coordinates[0];
      var polygon;
      ...
      if (layer.EPSG === undefined || layer.EPSG === bootleaf.mapWkid) {
        ...
        try {
          ...
          console.log(
            "Please ensure that ",
            layer.id,
            "'s coordinate system is added to config.projections"
          );
        }
      }

      queryData.CQL_FILTER +=
        " and INTERSECTS(" + geomField + ", POLYGON((" + polygon + ")))";
    }
...
function handleQueryResults(data, layerConfig, outFields) {
  ...
  if (
    ...
    data.spatialReference.wkid !== undefined
  ) {
    bootleaf.queryResults["wkid"] = data.spatialReference.wkid;
  } else if (
    data.crs !== undefined &&
    data.crs.properties !== undefined &&
    data.crs.properties.name !== undefined
  ) {
    var crs = data.crs.properties.name;
    bootleaf.queryResults["wkid"] = parseInt(
      crs.substr(crs.lastIndexOf(":") + 1)
    );
  }
  ...
  try {
    var dtConfig = {
      ...
      fnDrawCallback: function () {
        if (
          $(".dataTables_paginate .paginate_button").length &&
          $(".dataTables_paginate .paginate_button").length > 3
        ) {
    ...
    console.log(
      "There was a problem enabling DataTables for the Query Widget results",
      err
    );
  }
  ...
  try {
    ...
    console.log(
      "There was a problem enabling Data Tables for the Query Widget buttons",
      err
    );
  }
}
...
function resetQueryOutputTable() {
  $("#queryResults").html(
    '<span id="ajaxLoading"></span><table id="tblQueryResults" class="table table-condensed table-hover"></table><div id="exportButtons"></div>'
  );
}
...
function configureFilterWidget() {
  ...
  if (bootleaf.filterTasks) {
    if (bootleaf.filterTasks.length > 0) {
      ...
      $("#sidebarContents").html(
        "<p><span class='info'>There are no filter-able layers in the map</span></p>"
      );
...
function updateFilterParams(control) {
  ...
  var layerId = $("#filterWidgetLayer option:selected").val();
  var fieldName = $("#filterWidgetField option:selected").val();
  var fieldType = $("#filterWidgetField option:selected")[0].dataset[
    "fieldtype"
  ];
  var operator = $("#filterWidgetOperator option:selected").val();
  ...
  if (filter) {
    if (filter.value !== undefined && filter.operator !== undefined) {
      ...
      if (control !== "operator") {
        $('#filterWidgetOperator option[value="' + filter.operator + '"]').attr(
          "selected",
          "selected"
        );
      }
  ...
  for (var i = 0; i < filterTask.filters.length; i++) {
    ...
    if (filter.operator && filter.value) {
      if (!filterText) {
        ...
        filterText +=
          " and " + filter.alias + " " + filter.operator + " " + filter.value;
      }
...
function applyFilter() {
  ...
  for (var layerIdx = 0; layerIdx < config.layers.length; layerIdx++) {
    ...
    if (layerConfig.id === layerId) {
      ...
      if (
        layerConfig.type === "agsDynamicLayer" ||
        layerConfig.type === "agsFeatureLayer"
      ) {
        ...
        for (var filterIdx in filterTask.filters) {
          ...
          if (
            filterDef.value !== undefined &&
            filterDef.operator !== undefined
          ) {
            if (filterDef.type === "numeric") {
              ...
              if (filterDef.value === "*" || filterDef.value === "") {
                ...
                if (filter === undefined) {
                  ...
                  filter +=
                    " AND " + filterDef.name + " < '" + filterDef.value + "'";
                }
              } else if (
                filterDef.type === "date" &&
                filterDef.operator === ">"
        ...
        if (filter === undefined || filter.indexOf("undefined") > -1) {
          $.growl.warning({
            title: "Filter Widget",
            message:
              "There was a problem filtering this layer. Please check the parameters",
          });
          return;
        }
        ...
        if (layerConfig.type === "agsDynamicLayer") {
          try {
            ...
            $.growl.warning({
              title: "Filter Widget",
              message: "There was a problem filtering this layer",
            });
            console.error(error);
          }
...
function configureIdentifyTool() {
  ...
  if (bootleaf.identifyLayers.length === 0) {
    $("#sidebarContents").html(
      "<p><span class='info'>There are no identifiable layers currently visible on the map</span></p>"
    );
    disableIdentify();
  } else {
    $("#sidebarContents").html(
      "<p><span class='info'>Click on the map to identify visible layers</span></p>"
    );
    enableIdentify();
  }
}
  ...
  for (var idx = 0; idx < bootleaf.identifyLayers.length; idx++) {
    ...
    if (idLayer.layerConfig.type === "agsDynamicLayer") {
      ...
      var data = {
        ...
        maxAllowableOffset:
          idLayer.layerConfig.identify.maxAllowableOffset || 0.1,
        returnGeometry: true,
        imageDisplay:
          bootleaf.map.getSize().x + "," + bootleaf.map.getSize().y + "," + 96,
        mapExtent: mapExtent,
        geometry: geometry,
      ...
      try {
        if (idLayer.layerConfig.where !== undefined) {
          data["layerDefs"] =
            "{" +
            idLayer.layerConfig.layers[0] +
            ":" +
            idLayer.layerConfig.where +
            "}";
        }
      } catch (err) {
        console.log(
          "There was a problem applying the Where clause to the Identify task"
        );
      }
      ...
      $.when(
        ...
        function (data, textStatus, jqXHR) {
          ...
          if (data.error !== undefined) {
            $.growl.error({
              message: "There was an error with the Identify function:",
            });
            ...
            $.each(data.results, function (index, result) {
              ...
              var layerName =
                identify.layerName || layerConfig.name || layerConfig.id;
              ...
              if (
                bootleaf.identifyResponse[layerId][value] === undefined &&
                ...
                if (layerConfig.identify.maxFeatures !== undefined) {
                  ...
                  if (
                    bootleaf.identifyCounter[layerName] <=
                    layerConfig.identify.maxFeatures
                  ) {
        ...
        function (error) {
          $("#sidebarContents").html(
            "<p><span class='info'>There was an error with the Identify tool</span></p>"
          );
        }
      );
      ...
      var width =
        bootleaf.map.getBounds().getEast() - bootleaf.map.getBounds().getWest();
      ...
      $.when(
        ...
        function (data, textStatus, jqXHR) {
          ...
          if (data.features.length > 0) {
            var layerId = data.features[0].id.substring(
              0,
              data.features[0].id.indexOf(".")
            );
            var layerConfig = jqXHR.layerConfig;
            for (var i = 0; i < bootleaf.identifyLayers.length; i++) {
              ...
              if (layerConfig.identify.layerName === layerId) {
                var layerName =
                  layerConfig.name || layerConfig.id || "unknown layer";
...
function displayIdentifyResult(layerId, layerName, layerConfig, result) {
  ...
  if (bootleaf.identifyLayerHeadings.indexOf(layerName) < 0) {
    ...
    sidebarContents +=
      "<div class='identifyResults'><strong>" +
      layerConfig.name +
      "</strong><ul class='identifyLayer list-unstyled list-group' id='" +
      layerConfig.id +
      "'></ul></div>";
  ...
  output +=
    "<li class='list-group-item identifyResult' data-guid=" + guid + ">";
  if (layerConfig.identify.primaryField !== undefined) {
    output +=
      "<span class='primaryField'>" +
      result.attributes[layerConfig.identify.primaryField] +
      "</span><br>";
  }
  if (outFields.length > 0) {
    for (var fldIdx = 0; fldIdx < outFields.length; fldIdx++) {
...
function configureCoordinatesTool() {
  ...
  resetSidebar(
    "Map coordinates",
    "<p><span class='info'>Click on the map to view the coordinates</span></p>"
  );
  $("#sidebar").show("slow");
  switchOffTools();
...
function getAllUrlParams() {
  ...
  if (queryString) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (
        $.inArray(paramName, ["lat", "lng", "zoom", "basemap", "layers"]) < 0
      ) {
...
function fetchWFS() {
  ...
  for (var j = 0; j < bootleaf.wfsLayers.length; j++) {
    ...
    if (layer.lastRun) {
      ...
      if (elapsed < 500) {
        console.log(
          "sequential WFS requests are appearing too soon - rate limiting"
        );
...
function wfsAjax(layer) {
  ...
  if (layerConfig.callback !== undefined) {
    callback = layerConfig.callback;
  }
  var data = {
    service: "WFS",
    version: "1.0.0",
  ...
  if (
    layerConfig.fields &&
    layerConfig.fields.length &&
    layerConfig.fields.length > 0
  ) {
  ...
  try {
    ...
    for (var j = 0; j < data.features.length; j++) {
      ...
      if (newData.latLngs.indexOf(latLng) === -1) {
        ...
        feature.properties["_xxxLabelText"] =
          feature.properties[layerConfig.label.name];
    ...
    for (var i = 0; i < bootleaf.labelLayers.length; i++) {
      var labelLayer = bootleaf.labelLayers[i];
      if (
        bootleaf.labelLayers[i].layerConfig.id ===
        layerConfig.id + "_labels"
      ) {
...
function authoriseArcGIS(e) {
  ...
  serverAuth(config.tokenUrl, username, password, function (error, response) {
    if (error) {
      ...
      $.growl.error({
        message: "There was a problem logging in to ArcGIS Online",
        fixed: true,
      });
      return;
    }
